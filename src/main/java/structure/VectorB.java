...
public class VectorB {
	...
	public VectorB hadamardProd(VectorB v) {
		return VectorB.from(Streams.zip(this.list.stream(), v.list.stream(), (b1, b2) -> b1.multiply(b2).mod(this.n))
				.collect(Collectors.toList()), this.n);
	}
	...
	public static VectorB randoms(int size, BigInteger n) {
		return new VectorB(
				IntStream.range(0, size).mapToObj(i -> CryptoGenerator.getRandomModQ(n)).collect(Collectors.toList()),
				n);
	}
	...
	public BigInteger sum() {
		return this.list.stream().reduce(BigInteger.ZERO, (b1, b2) -> b1.add(b2)).mod(this.n);
	}
	...
	public VectorB negate() {
		return new VectorB(this.list.stream().map(v -> v.negate().mod(this.n)).collect(Collectors.toList()), this.n);
	}
